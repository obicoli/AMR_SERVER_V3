...
class ReturnAssetControler extends Controller
{
    ...
    public function __construct( SupplierAsset $supplierAsset )
    {
        ...
        $this->supplierReturn = new SupplierRepository( new SupplierReturn() );
    ...
    public function create( Request $request ){
        ...
        try{
            ...
            $supplierReturn = $this->supplierReturn->findByUuid($request->attachable_id);
            $doc_tmp_name = $_FILES['file']['tmp_name'];
            $file_type = $_FILES['file']['type'];
            $file_name = $_FILES['file']['name'];
            $file_size = $_FILES['file']['size'];
            $doc_type = Product::DOC_PURCHASE_RETURN;
            $root_directory_array = $this->helper->create_upload_directory($company->uuid,$doc_type,$supplierReturn->trans_number);
            ...
            if( $this->helper->is_file_exist($newFilePath) ){
                ...
                $asset = $supplierReturn->assets()->create($inputs);
                $status['status'] = Product::STATUS_ATTACHED;
                $status['notes'] = "File attached to ".$doc_type;
                $status['type'] = "action";
                $return_status = $company_user->return_status()->create($status);
                $return_status = $supplierReturn->trails()->save($return_status);
            }
