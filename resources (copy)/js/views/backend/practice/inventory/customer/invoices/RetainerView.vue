        ...
        <div class="wp-content content-calculated-height-wc">
                            ...
                            <div class="estimate-list width-100-pc content-calculated-height-50">
                                <div class="estimate-list-head">
                                    ...
                                    <div class="float-left width-70-pc">
                                        <div  class="btn-group" role="group" aria-label="Button group">
                                            ...
                                            <div v-else class="btn-group" role="group">
                                                <a :id="'btnGroupDrop2_1'" class="dropdown-toggle pointer-cursor fw-600 cl-444 fs-14" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                    {{default_filter}} Retainer Invoices({{pagination.total}})
                                                </a>
                                                <div class="dropdown-menu" :aria-labelledby="'btnGroupDrop2_1'">
                                                    <!-- <a @click="filterInvoices('All')" class="dropdown-item pointer-cursor">All</a>
                                                    <a @click="filterInvoices(PROCESS_STATUS.DRAFT)" class="dropdown-item pointer-cursor">{{PROCESS_STATUS.DRAFT}}</a>
                                                    <a @click="filterInvoices(PROCESS_STATUS.PENDING_APPROVAL)" class="dropdown-item pointer-cursor">{{PROCESS_STATUS.PENDING_APPROVAL}}</a>
                                                    <a @click="filterInvoices(PROCESS_STATUS.OPEN)" class="dropdown-item pointer-cursor">{{PROCESS_STATUS.OPEN}}</a>
                                                    <a @click="filterInvoices(PROCESS_STATUS.OVERDUE)" class="dropdown-item pointer-cursor">{{PROCESS_STATUS.OVERDUE}}</a>
                                                    <a @click="filterInvoices(PROCESS_STATUS.UNPAID)" class="dropdown-item pointer-cursor">{{PROCESS_STATUS.UNPAID}}</a>
                                                    <a @click="filterInvoices(PROCESS_STATUS.PARTIAL_PAID)" class="dropdown-item pointer-cursor">{{PROCESS_STATUS.PARTIAL_PAID}}</a>
                                                    <a @click="filterInvoices(PROCESS_STATUS.PAID)" class="dropdown-item pointer-cursor">{{PROCESS_STATUS.PAID}}</a>
                                                    <a @click="filterInvoices(PROCESS_STATUS.VOID)" class="dropdown-item pointer-cursor">{{PROCESS_STATUS.VOID}}</a>
                                                    <a class="dropdown-item separator"></a>
                                                    ...
                                                    <a class="dropdown-item pointer-cursor">Paid</a>
                                                    <a class="dropdown-item pointer-cursor">Partially Drawn</a>
                                                    <a class="dropdown-item pointer-cursor">Drawn</a>
                                    ...
                                    <div class="width-20-pc float-right">
                                        <button type="button" data-toggle="modal" title="New Retainer Invoice" data-target="#create_retainer_invoice_modal" class="float-right btn btn-secondary banking-process-amref">+</button>
                                    </div>
                                </div>
                                        ...
                                        <tbody v-if="!customer_invoices.length">
                                            <tr class="parent">
                                                <td colspan="3" class="text-center cl-amref">
                                                    No Retainer Invoice to display!
                                                </td>
                                            </tr>
                                        </tbody>
                                        <tbody v-for="(customer_invoic,index) in customer_invoices" v-bind:class="{'active':active_po===index}" :key="'po_list_'+index">
                                            ...
                                            <tr class="child">
                                                ...
                                                <td style="width:35%" class="text-right">
                                                    <a @click="setInvoice(customer_invoic,index)" v-if="check_status(customer_invoic.status,PROCESS_STATUS.ACCEPTED)" class="txt-uppercase fs-12 fw-600 cl-success-light">Accepted</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else-if="check_status(customer_invoic.status,PROCESS_STATUS.DRAFT)" class="documentStatus draft">Draft</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else-if="check_status(customer_invoic.status,PROCESS_STATUS.DECLINED)" class="txt-uppercase fs-12 fw-600 cl-amref">Declined</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else-if="check_status(customer_invoic.status,PROCESS_STATUS.EXPIRED)" class="txt-uppercase fs-12 fw-600 cl-787887">Expired</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else-if="check_status(customer_invoic.status,PROCESS_STATUS.INVOICED)" class="txt-uppercase fs-12 fw-600 cl-success-light">Invoiced</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else-if="check_status(customer_invoic.status,PROCESS_STATUS.SENT)" class="txt-uppercase fs-12 fw-600 cl-blue-link">Sent</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else-if="check_status(customer_invoic.status,PROCESS_STATUS.PAID)" class="documentStatus paid">{{PROCESS_STATUS.PAID}}</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else-if="check_status(customer_invoic.status,PROCESS_STATUS.PARTIAL_PAID)" class="documentStatus paid">{{PROCESS_STATUS.PARTIAL_PAID}}</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else-if="check_status(customer_invoic.status,PROCESS_STATUS.UNPAID)" class="documentStatus unpaid">{{PROCESS_STATUS.UNPAID}}</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else-if="check_status(customer_invoic.status,PROCESS_STATUS.OPEN)" class="documentStatus open">Open</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else-if="check_status(customer_invoic.status,PROCESS_STATUS.CLOSED)" class="txt-uppercase fs-12 fw-600 cl-success-light">{{PROCESS_STATUS.CLOSED}}</a>
                                                    <a @click="setInvoice(customer_invoic,index)" v-else="" class="documentStatus draft">Pending Approval</a>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                            ...
                            <div class="box box-primary bg-white border-top-0 mg-bottom-0 content-calculated-height-50 border-bottom-0 no-shadowed">
                                <div  class="width-100-pc action-header padding-left-20 padding-top-10 padding-bottom-10">
                                    ...
                                    <div class="float-right width-80-pc text-right padding-right-30">
                                        ...
                                        <button v-if="page_ready && check_status(customer_invoice.status,PROCESS_STATUS.OPEN) || check_status(customer_invoice.status,PROCESS_STATUS.PARTIAL_PAID)" type="button" data-toggle="modal" data-target="#new_customer_receipt_modal" class="btn btn-secondary banking-process-amref">Record Payment</button>
                                        <button v-if="page_ready && check_status(customer_invoice.status,PROCESS_STATUS.PAID)" type="button" data-toggle="modal" data-target="#new_customer_receipt_modal" class="btn btn-secondary banking-process-amref">Apply to Invoice</button>
                                        <button v-if="page_ready && check_status(customer_invoice.status,PROCESS_STATUS.PARTIAL_PAID) || check_status(customer_invoice.status,PROCESS_STATUS.PAID)" type="button" data-toggle="modal" data-target="#" class="btn btn-secondary banking-process-amref">Refund</button>
                                        <span v-else></span>
                                        <div v-if="page_ready" class="btn-group" role="group" aria-label="Button group with nested dropdown">
                                            <div class="btn-group" role="group">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="btnGroupDrop">
                                                    <a class="dropdown-item pointer-cursor" data-toggle="modal" data-target="#send_po_mail_00">Record Payment</a>
                                                    <a v-if="check_status(customer_invoice.status,PROCESS_STATUS.PAID)" class="dropdown-item pointer-cursor" data-toggle="modal" data-target="#change_status_modal">Apply to Invoice</a>
                                                    <a class="dropdown-item separator"></a>
                                        ...
                                        <router-link :to="RETAINER_INVOICES" tag="i" class="btn btn-default btn-inventory btn-gray"><i class="fa fa-arrow-circle-left"></i> Back</router-link>
                                    </div>
                                ...
                                <div class="page-content bg-white padding-left-0 height-scrollable">
                                                ...
                                                <div class="width-100-pc float-left">
                                                    <nav>
                                                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                                            <a v-bind:class="{'nav-item nav-link nav-link-role':true,'active':curr_tab===0}" :id="'nav-transactions-tab-'+'0'" data-toggle="tab" :href="'#nav-transactions-'+'0'" role="tab" :aria-controls="'nav-transactions-'+'0'" aria-selected="true" :key="'transactions_tab'+'0'">{{customer_invoice.trans_number}}</a>
                                                        ...
                                                        <div v-bind:class="{'tab-pane fade':true,'show active':curr_tab===0}" :id="'nav-transactions-'+'0'" role="tabpanel" :aria-labelledby="'nav-transactions-tab-'+'0'" :key="'tab_index_'+'0'">
                                                            <div class="width-100-pc float-left padding-left-15 padding-right-10 bg-f4 padding-bottom-20">
                                                                <section v-if="page_ready" class="padding-right-0 padding-left-0 padding-top-20" id="print_section">
                                                                    ...
                                                                    <retainer-invoice-preview v-else :currency="currency" :customer_invoice="customer_invoice"></retainer-invoice-preview>
                                                                </section>
                                                            </div>
                                                        </div>
                                                        <div v-if="customer_invoice.payments.length" v-bind:class="{'tab-pane fade':true,'show active':curr_tab===1}" :id="'nav-transactions-'+'1'" role="tabpanel" :aria-labelledby="'nav-transactions-tab-'+'1'" :key="'tab_index_'+'1'">
                                                            <div class="width-100-pc float-left padding-left-15 padding-right-10 bg-f4 padding-bottom-20">
                                                                <section v-if="page_ready" class="padding-right-0 padding-left-0 padding-top-20" id="print_section">
                                                                    ...
                                                                    <payment-received-preview v-for="(paymentdone,index_p) in customer_invoice.payments" :payment_received="paymentdone" :currency="currency" :balance="calc_balance(customer_invoice.net_total,customer_invoice.payments,index_p)" :show_actions="true" :key="'payment_done_'+index_p"></payment-received-preview>
                                                                </section>
                                                            </div>
                                                        </div>
                                 ...
                                 <change-status v-if="page_ready" @change-status-event="loadBills(true)" :modal_title="'Change Status'" :item_url="SUPPLIER_BILL_URL+'/'+bill.id" :modal_id="'change_status_modal'"></change-status> -->
            ...
            <div class="row justify-content-center">
                <div class="col-md-12 col-lg-12 col-sm-12 padding-left-0">
                    <customer-receipt-modal v-if="page_ready" :filters="filters" :retainer_invoices="customer_invoice" :payment_api="CUSTOMERS_PAYMENTS_API" :currency="currency" :accounts="accounts" :bank_accounts="bank_accounts" :modal_id="'new_customer_receipt_modal'" :modal_title="'Payment for '+customer_invoice.trans_number" @create-payment-event="loadInvoices(true)"></customer-receipt-modal>
                    <retainer-invoice-modal v-if="page_ready" :modal_title="'New Retainer Invoice'" :customers="customers" :modal_id="'create_retainer_invoice_modal'" :bank_accounts="bank_accounts" :payment_terms="payment_terms" :filters="filters" :taxations="taxations" :currency="currency" :customer_retainer_invoice_api="CUSTOMERS_RETAINER_INVOICES_API" @create-retainer-event="loadInvoices(true)"></retainer-invoice-modal>
...
<script>
    ...
    import RetainerInvoiceModal from "../../../partials/modals/customer/RetainerInvoiceModal";
    ...
    import PaymentReceivedPreview from "../../../partials/previews/supplychain/customer/PaymentReceivedPreview";
    import InvoicePreview from "../../../partials/previews/supplychain/customer/InvoicePreview";
    import RetainerInvoicePreview from "../../../partials/previews/supplychain/customer/RetainerInvoicePreview";
    ...
    import {CUSTOMERS_INVOICES_API,CUSTOMERS_PAYMENTS_API,CHART_OF_ACCOUNTS,CUSTOMERS_RETAINER_INVOICES_API,BANKING_ACCOUNTS_URL,PRODUCT_TAX_URL,CUSTOMERS_SALES_ORDERS_API,CUSTOMERS_ESTIMATES_API, CUSTOMERS_API, CUSTOMERS_TERMS_API} from '../../../../../../router/api_routes';
    ...
    export default {
        ...
        SideBarInventory,CopyRight,SearchDatabaseFormControl,ChangeStatus,RetainerInvoiceModal,InvoicePreview,
        FileAttachModal,CustomerReceiptModal,RetainerInvoicePreview,PaymentReceivedPreview},
        data(){
            return {
                curr_tab: 0,
                ...
                RETAINER_INVOICES: INVENTORY_WEB_ROUTES.SALES.RETAINER_INVOICES,
                invoices: [],
                bank_accounts: [],
                customer_invoices: [],
                customer_invoice: null,
                default_filter: 'All',
                default_api: CUSTOMERS_RETAINER_INVOICES_API+'?page=1',
                ...
                CUSTOMERS_RETAINER_INVOICES_API: CUSTOMERS_RETAINER_INVOICES_API,
                initial_invoice_api: CUSTOMERS_RETAINER_INVOICES_API+'/'+this.$route.params.uuid,
            }
        },
        ...
        methods: {
            ...
            addComment(){
                ...
                let api_ = CUSTOMERS_RETAINER_INVOICES_API+'/'+this.customer_invoice.id+'?action='+FORM_ACTIONS.COMMENT;
            ...
            setInvoice(bill_obj,index=-1){
                ...
                this.initial_invoice_api = CUSTOMERS_RETAINER_INVOICES_API+'/'+bill_obj.id;
                this.loadInvoice();
            },
            ...
            log_print(element_id,msg){
                this.printing = true;
                let api_ = CUSTOMERS_RETAINER_INVOICES_API+'/'+this.customer_invoice.id+'?action='+FORM_ACTIONS.PRINT;
                let form_ = {};
                post(api_,form_)
                    .then((res) => {
                        if(res.data.status_code === 200) {
            ...
            filterInvoices(filter_by='All'){
                switch (filter_by) {
                    ...
                    case PROCESS_STATUS.PARTIAL_PAID:
                        ...
                        this.default_api = CUSTOMERS_RETAINER_INVOICES_API+'?page='+this.pagination.current_page+'&filter_by='+this.default_filter;
                        break;
                    case "Page":
                        if(this.default_filter==='All'){
                            this.default_api = CUSTOMERS_RETAINER_INVOICES_API+'?page='+this.pagination.current_page;
                        }else{
                            this.default_api = CUSTOMERS_RETAINER_INVOICES_API+'?page='+this.pagination.current_page+'&filter_by='+this.default_filter;
                        }
                        break;
            ...
            loadInvoice(){
                ...
                .then((res) => {
                    if(res.data.status_code === 200) {
                        ...
                        //this.mailing_api = CUSTOMERS_RETAINER_INVOICES_API+'/'+this.customer_invoice.id+'?action='+FORM_ACTIONS.SEND_MAIL;
            ...
            loadAccounts(){
                get(CHART_OF_ACCOUNTS+'?account_nature=Assets')
                .then((res) => {
                    if(res.data.status_code === 200) {
