                ...
                <div class="col-md-12 col-lg-12 col-sm-12 padding-left-0">
                    ...
                    <div class="row mg-top-35">
                        ...
                        <div class="col-md-2 col-lg-2 col-sm-2 padding-right-0 bg-foo">
                            <side-bar-inv-settings v-if="page_ready" :scrollable="false" :width="true" :transparent="true" :active_el="'categories'" :title="'Inventory Settings'"></side-bar-inv-settings>
                        </div>
                        <div class="col-md-10 col-lg-10 col-sm-10 padding-right-0 padding-left-0 bg-foo">
                            ...
                            <div class="box box-primary bg-white border-top-0 border-bottom-0 no-shadowed">
                                <div class="page-content bg-white min-height-100-vh padding-10">
                                    <div class="dgrid dgrid-grid ui-widget dgrid-03 mg-bottom-30 universal-grid border-ccc padding-left-20 padding-right-20 mg-top-45" role="grid">
                                        <div class="width-70-pc pull-left mg-bottom-10">
                                            <router-link :to="INVENTORY_WEB_ROUTES.INV_SET_ORDER_CATEGORIES" class="btn-approved-md pointer-cursor float-right mg-right-55">Order Category</router-link>
                                            ...
                                            <router-link :to="INVENTORY_WEB_ROUTES.INV_SET_CATEGORIES" class="pointer-cursor btn-default-md float-right mg-right-5">Category</router-link>
                                            <search-database-form-control v-if="page_ready" :search_type="'Order Category'" :placeholder="'search by name'" v-on:searchedData="refresh_data" :practice_id="practice_id" :field_wdth="250" :rounded_px="10"></search-database-form-control>
                                        </div>
                                        <div class="width-20-pc pull-right mg-bottom-10">
                                            <div class="btn-group float-right" role="group" aria-label="Button group with nested dropdown">
                                                <button data-toggle="modal" data-target="#new_order_category_modal" type="button" class="btn combo-button combo-default">+ Order Category</button>
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="btnGroupDrop">
                                                        <a class="dropdown-item" data-toggle="modal" data-target="#import_order_categories_modal_id"> Import Order Category</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <table class="receipt-item-table-2 table table-bordered table-hover mg-top-5">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width: 15%;">
                                                        <a @click="loadOrderCategories" title="Refresh" class="fa fa-refresh float-right pointer-cursor"></a>
                                                    </th>
                                                </tr>
                                            </thead>
                                            <tbody v-if="categories.length">
                                                <tr v-for="(category, index) in categories" :key="'category_'+index" class="pointer-cursor">
                                                    ...
                                                    <delete-modal :modal_title="'Delete Category!'" :modal_id="'delete_category_'+index" v-on:deletionSuccessful="remove_el(category)" :confirm_message="'Are you sure?'" :item_url="initial_url+'/'+category.id"></delete-modal>
                                                    <order-category-modal v-if="page_ready" :form_data="category" :INITIAL_URL="initial_url+'/'+category.id" :item_id="category.id" :user_mode="'Edit'" :title="'Edit Order Category'" :modal_id="'new_category_modal_'+index" v-on:categoryEdited="replace_edited" :practice_id="practice_id"></order-category-modal>
                                                </tr>
                                            </tbody>
                                            <tbody v-if="!categories.length">
                                                <tr>
                                                    <td colspan="5" class="cl-amref text-center fs-16 padding-top-20 padding-bottom-20">No order category to dispalay</td>
                                                </tr>
                                            </tbody>
                                            <tfoot>
                                                <tr>
                                                    <th colspan="5" class="bg-foo">
                                                        <paginate-template v-if="pagination.last_page > 1" :pagination="pagination" :offset="5" @paginate="loadOrderCategories()" :custom="true"></paginate-template>
                                                    </th>
                                                </tr>
                                            </tfoot>
                                        </table>
                                ...
                                <order-category-modal v-if="page_ready" :user_mode="'Create'" :INITIAL_URL="initial_url" :title="'+ New Order Category'" :modal_id="'new_order_category_modal'" v-on:categoryAdded="loadOrderCategories" :practice_id="practice_id"></order-category-modal>
                                <general-import-modal v-if="page_ready" v-on:imported="loadOrderCategories" :practice_id="practice_id" :title="'Import Order Category'" :modal_id="'import_order_categories_modal_id'" :modal_min_width="900" :sample_file="'ordercategories'" :upload_type="'Order Category'"></general-import-modal>
                            </div>
...
<script>
    ...
    import OrderCategoryModal from "../../partials/modals/inventory_settings/category/OrderCategoryModal";
    ...
    import {PRODUCT_ATTRIBUTES_URL,PRODUCT_ORDER_CATEGORIES_URL} from '../../../../../router/api_routes';
    import {INVENTORY_WEB_ROUTES} from '../../../../../router/web_routes';
    export default {
        name: "Index",
        components: {TopNavBar, SideBar,SideBarInvSettings,
        DeleteModal,OrderCategoryModal,GeneralImportModal,SearchDatabaseFormControl,
        ...
        data(){
            return {
                ...
                page_ready: false,
                practice_id: '',
                initial_url: PRODUCT_ORDER_CATEGORIES_URL,
        ...
        methods: {
            ...
            loadOrderCategories(){
                this.categories = [];
                this.progress_overlay = "show";
                get(PRODUCT_ORDER_CATEGORIES_URL+'/practice/'+this.practice_id+'?page='+this.pagination.current_page)
                .then((res) => {
                    if(res.data.status_code === 200) {
