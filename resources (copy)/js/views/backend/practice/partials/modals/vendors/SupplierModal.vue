                        ...
                        <div class="width-100-pc float-left mg-top-30">
                            <nav>
                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                    ...
                                    <a @click="toggleTab(11)" v-bind:class="{'nav-item nav-link nav-link-role':true,'active':curr_tab===11}" :id="'nav-transactions-tab-'+'11'" data-toggle="tab" :href="'#nav-transactions-'+'11'" role="tab" :aria-controls="'nav-transactions-'+'11'" aria-selected="true" :key="'transactions_tab'+'11'">Banking Details</a>
                            ...
                            <div class="tab-content tab-content-role" id="nav-tabContent">
                                ...
                                <div v-bind:class="{'tab-pane fade':true,'show active':curr_tab===11}" :id="'nav-transactions-'+'11'" role="tabpanel" :aria-labelledby="'nav-transactions-tab-'+'11'" :key="'tab_index_'+'11'">
                                    <!-- Tab 1 -->
                                    <div class="width-100-pc float-left padding-left-15 padding-right-15 bg-f4 padding-bottom-20">
                                        <!-- Tab 1 Contact Details -->
                                        <div class="width-45-pc float-left mg-top-20">
                                            <div class="row fullName mg-bottom-5 mg-left-30">
                                                <div class="inlineBlock width-100-pc">
                                                    <label class="fullname fw-600 fs-14">Supplier Banking Details</label>
                                                </div>
                                            </div>
                                            <div class="row fullName mg-bottom-5 mg-left-30">
                                                <div class="inlineBlock width-40-pc">
                                                    <label class="company-label text-right fs-12">Account Holder:&nbsp;&nbsp;</label>
                                                </div>
                                                <div class="inlineBlock width-60-pc">
                                                    <div class="dijitInline firstName dijitTextBox width-100-pc">
                                                        <input v-model="form.account_name" required v-bind:class="{'height-32':true,'custom-attended-field':form.account_name}" type="text">
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="row fullName mg-bottom-5 mg-left-30">
                                                <div class="inlineBlock width-40-pc">
                                                    <label class="company-label text-right fs-12">Account Number:&nbsp;&nbsp;</label>
                                                </div>
                                                <div class="inlineBlock width-60-pc">
                                                    <div class="dijitInline firstName dijitTextBox width-100-pc">
                                                        <input v-model="form.account_number" required v-bind:class="{'height-32':true,'custom-attended-field':form.account_number}" type="text">
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="row fullName mg-bottom-5 mg-left-30">
                                                <div class="inlineBlock width-40-pc">
                                                    <label class="company-label text-right fs-12">Bank Name:&nbsp;&nbsp;</label>
                                                </div>
                                                <div class="inlineBlock width-60-pc">
                                                    <div class="dijitInline firstName dijitTextBox width-100-pc">
                                                        <select v-model="form.bank_id" @change="setBranch($event)" v-bind:class="{'height-32':true,'custom-attended-field':form.bank_id}">
                                                            <option value="">-select-</option>
                                                            <option v-for="(bank,index) in banks" :value="bank.id" :key="'banks'+index">{{bank.name}}</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="row fullName mg-bottom-5 mg-left-30">
                                                <div class="inlineBlock width-40-pc">
                                                    <label class="company-label text-right fs-12">Branch Name:&nbsp;&nbsp;</label>
                                                </div>
                                                <div class="inlineBlock width-60-pc">
                                                    <div class="dijitInline firstName dijitTextBox width-100-pc">
                                                        <select v-model="form.bank_branch_id" v-bind:class="{'height-32':true,'custom-attended-field':form.bank_branch_id}">
                                                            <option value="">-select-</option>
                                                            <option v-for="(bank_branch,index) in selected_bank_branches" :value="bank_branch.id" :key="'bank_branch'+index">{{bank_branch.name}}</option>
                                                        </select>
                                                        <!-- <input v-model="form.mobile" required v-bind:class="{'height-32':true,'custom-attended-field':form.mobile}" type="text"> -->
                                                    </div>
                                                </div>
                                            ...
                                            <div class="row fullName mg-bottom-5 mg-left-30">
                                                <div class="inlineBlock width-40-pc">
                                                    <label class="company-label text-right fs-12">Account Type:&nbsp;&nbsp;</label>
                                                </div>
                                                <div class="inlineBlock width-60-pc">
                                                    <div class="dijitInline firstName dijitTextBox width-100-pc">
                                                        <select v-model="form.account_type_id" v-bind:class="{'height-32':true,'custom-attended-field':form.account_type_id}">
                                                            <option value="">-select-</option>
                                                            <option v-for="(bank_account_type,index) in bank_account_types" :value="bank_account_type.id" :key="'bank_account_type'+index">{{bank_account_type.name}}</option>
                                                        </select>
                                                        <!-- <input v-model="form.fax" v-bind:class="{'height-32':true,'custom-attended-field':form.fax}" type="text"> -->
                                                    </div>
                                                </div>
                ...
                <div class="modal-footer">
                    <div v-if="user_mode==='Create'" class="btn-group float-right" role="group">
                        ...
                        <div class="dropdown-menu" :aria-labelledby="'btnGroupDrop2_1'">
                            <a @click="add_supplier(FORM_ACTIONS.SAVE_NEW)" class="dropdown-item pointer-cursor">{{FORM_ACTIONS.SAVE_NEW}}</a>
                            <a @click="add_supplier(FORM_ACTIONS.SAVE_CLOSE)" class="dropdown-item pointer-cursor">{{FORM_ACTIONS.SAVE_CLOSE}}</a>
                        </div>
                    </div>
                    <button v-else :disabled="processing" @click="add_supplier(FORM_ACTIONS.SAVE_CLOSE)" type="button" class="btn btn-secondary banking-process-amref">
                        Save
                    </button>
...
<script>
    ...
    export default {
        ...
        props:['form_data','filters','currency','suppliers_api','user_mode','modal_id','countries','vats','prices','supplier_terms','title','banks','bank_branches','bank_account_types'],
        data(){
            return {
                processing: false,
                curr_tab: 0,
        ...
        methods: {
            ...
            add_supplier: function (action_taken) {
                ...
                post(this.suppliers_api,this.form)
                    .then((res) => {
