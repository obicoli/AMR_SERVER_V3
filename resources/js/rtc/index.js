...
export let myHostname = window.location.hostname;
...
export let targetUsername = null;      // To store username of other peer
export let myPeerConnection = null;    // RTCPeerConnection
...
export function log_this(text) {
    let time = new Date();
    console.log("[" + time.toLocaleTimeString() + "] " + text);
}

export function showTraceable(conn, data) {
    let msgJSON = JSON.stringify(data);
    log_this("Sending '" + data.type + "' message: " + msgJSON);
    conn.send(msgJSON);
}

export function alertBeep() {
    let alert_beep = "http://localhost:8000/assets/sounds/sms-alert-1.mp3";
    let sound = new Audio(alert_beep);
    if (sound){
...
export function sendToServer(conn,msg) {
