                                    ...
                                    <div class="dgrid dgrid-grid ui-widget dgrid-03 mg-bottom-30 universal-grid border-ccc" role="grid">
                                        <div class="row justify-content-center">
                                            <div class="col-md-10 col-lg-10 padding-right-0 border-bottom">
                                                <div :hidden="customer_invoices.length===0 && default_filter==='All'" class="width-100-pc float-left">
                                                ...
                                                <div class="width-100-pc float-left mg-top-30 mg-bottom-50">
                                                    <div v-if="page_ready && customer_invoices.length===0" class="width-100-pc float-left mg-top-10 mg-bottom-50 text-center">
                                                        <div class="width-100-pc text-center float-left">
                                                            <a class="fs-15 txt-uppercase cl-444 fw-600">Retainer Invoice</a><br>
                                                            <!-- <a class="fs-14 cl-444">Get paid!</a><br> -->
                                                            <small class="fs-13 cl-777">Create a retainer to collect advance payments from your customers and apply them to multiple invoices.</small><br><br>
                                                            <a data-toggle="modal" data-target="#create_retainer_invoice_modal" class="btn btn-secondary banking-process-amref">Create Retainer Invoice</a>
                                                        </div>
                                                        <div class="width-100-pc text-center float-left mg-top-20">
                                                            <img src="/assets/icons/dashboard/retainer_invoice.png" class=" mg-bottom-20">
                                                        </div>
                                                    </div>
                                                    <div v-else-if="page_ready && customer_invoices.length>0" class="width-100-pc float-left mg-top-30 mg-bottom-50">
                                                        ...
                                                        <div class="width-25-pc float-right mg-bottom-15">
                                                            <div class="btn-group float-right" role="group">
                                                                ...
                                                                <div class="dropdown-menu" :aria-labelledby="'btnGroupDrop2_1'">
                                                                    ...
                                                                    <a data-toggle="modal" data-target="#create_retainer_invoice_modal" class="dropdown-item pointer-cursor">Retainer Invoice</a>
                                                            ...
                                                            <table class="table banking-transaction table-hover mg-bottom-5">        
                                                                <thead>
                                                                    <tr>
                                                                        ...
                                                                        <th style="width:12%;" class="text-right">Amount({{currency}})</th>
                                                                        <th style="width:12%;" class="text-right">Balance Due({{currency}})</th>
                                                                    </tr>
                                                                </thead>
                                                                ...
                                                                <tbody v-for="(customerinvoice,index) in customer_invoices" :key="'chart_of_accounts_'+index">
                                                                    <tr>
                                                                        <td class="somepad">
                                                                            <input :disabled="is_initializing || processing" v-model="selected_salesorders" type="checkbox" class="pointer-cursor" :value="customerinvoice.id">
                                                                        </td>
                                                                        <router-link tag="td" :to="RETAINER_INVOICES+'/'+customerinvoice.id+'/view'" class="somepad pointer-cursor">
                                                                            {{customerinvoice.customer.legal_name}}
                                                                        </router-link>
                                                                        <router-link tag="td" :to="RETAINER_INVOICES+'/'+customerinvoice.id+'/view'" class="somepad pointer-cursor">
                                                                            {{customerinvoice.trans_date}}
                                                                        </router-link>
                                                                        <router-link tag="td" :to="RETAINER_INVOICES+'/'+customerinvoice.id+'/view'" class="somepad pointer-cursor">
                                                                            {{customerinvoice.trans_number}}
                                                                        </router-link>
                                                                        <router-link tag="td" :to="RETAINER_INVOICES+'/'+customerinvoice.id+'/view'" class="somepad pointer-cursor">
                                                                            {{customerinvoice.reference_number}}
                                                                        </router-link>
                                                                        <!-- <router-link tag="td" :to="RETAINER_INVOICES+'/'+customerinvoice.id+'/view'" class="somepad pointer-cursor">
                                                                            {{customerinvoice.due_date}}
                                                                        </router-link> -->
                                                                        <router-link tag="td" :to="RETAINER_INVOICES+'/'+customerinvoice.id+'/view'" class="somepad pointer-cursor">
                                                                            <a v-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.ACCEPTED" class="txt-uppercase fs-12 fw-600 cl-success-light">Accepted</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.DRAFT" class="documentStatus draft">{{PROCESS_STATUS.DRAFT}}</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.UNPAID" class="documentStatus unpaid">{{PROCESS_STATUS.UNPAID}}</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.DECLINED" class="txt-uppercase fs-12 fw-600 cl-amref">Declined</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.EXPIRED" class="txt-uppercase fs-12 fw-600 cl-787887">Expired</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.INVOICED" class="txt-uppercase fs-12 fw-600 cl-success-light">Invoiced</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.PAID" class="documentStatus paid">{{PROCESS_STATUS.PAID}}</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.PARTIAL_PAID" class="documentStatus paid">{{PROCESS_STATUS.PARTIAL_PAID}}</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.SENT" class="txt-uppercase fs-12 fw-600 cl-blue-link">Sent</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.OPEN" class="documentStatus open">{{PROCESS_STATUS.OPEN}}</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.CLOSED" class="documentStatus closed">{{PROCESS_STATUS.CLOSED}}</a>
                                                                            <a v-else-if="customerinvoice.status[customerinvoice.status.length-1].status===PROCESS_STATUS.PENDING_APPROVAL" class="documentStatus pending">{{PROCESS_STATUS.PENDING_APPROVAL}}</a>
                                                                            <a v-else="" class="documentStatus pending">Pending</a>
                                                                        </router-link>
                                                                        <router-link tag="td" :to="RETAINER_INVOICES+'/'+customerinvoice.id+'/view'" class="somepad pointer-cursor text-right fw-600">
                                                                            {{format_money(customerinvoice.net_total)}}
                                                                        </router-link>
                                                                        <router-link tag="td" :to="RETAINER_INVOICES+'/'+customerinvoice.id+'/view'" class="somepad pointer-cursor text-right fw-600">
                                                                            {{format_money(customerinvoice.due_balance)}}
                                                                        </router-link>
                                                                    </tr>
                                                                </tbody>
                                                            </table>
                                        ...
                                        <div class="row justify-content-center">
                                            <div class="col-md-12 col-lg-12 padding-right-0 border-bottom">
                                                ...
                                                <invoice-modal v-if="page_ready" :modal_title="'New Tax Invoice'" :customers="customers" :modal_id="'create_invoice_modal'" :bank_accounts="bank_accounts" :payment_terms="payment_terms" :filters="filters" :taxations="taxations" :currency="currency" :customer_invoice_api="CUSTOMERS_RETAINER_INVOICES_API" @create-salesorder-event="goTo(INVOICES_URL)"></invoice-modal>
...
<script>
    ...
    import {CUSTOMERS_SALES_ORDERS_API,CUSTOMERS_RETAINER_INVOICES_API,PRODUCT_TAX_URL,CHART_OF_ACCOUNTS, CUSTOMERS_API,CUSTOMERS_TERMS_API,CUSTOMERS_ESTIMATES_API, BANKING_ACCOUNTS_URL} from '../../../../../../router/api_routes';
    export default {
        name: "Index",
        components: {TopNavBar,PaginateTemplate,SideBar,DeleteModal,CopyRight,EstimateModal,SalesOrderModal,
        ProcessingOverlay,InvoiceModal,RetainerInvoiceModal},
        data(){
            return {
