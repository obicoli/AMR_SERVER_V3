                    ...
                    <div class="row mg-top-35">
                        ...
                        <div class="col-md-10 col-lg-10 col-sm-10 padding-right-0 padding-left-0 bg-foo">
                            ...
                            <div class="box box-primary bg-white border-top-0 border-bottom-0 no-shadowed">
                                <div class="page-content bg-white min-height-100-vh padding-10">
                                    <div class="dgrid dgrid-grid ui-widget dgrid-03 mg-bottom-30 universal-grid border-ccc padding-left-20 padding-right-20 mg-top-45" role="grid">
                                        <div class="width-70-pc pull-left mg-bottom-10">
                                            ...
                                            <router-link :to="INVENTORY_WEB_ROUTES.INV_SET_SUB_CATEGORIES" class="btn-approved-md pointer-cursor float-right mg-right-5">Sub Category</router-link>
                                            <router-link :to="INVENTORY_WEB_ROUTES.INV_SET_CATEGORIES" class="btn-default-md pointer-cursor float-right mg-right-5">Category</router-link>
                                            <search-database-form-control v-if="page_ready" :placeholder="'search here'" :search_type="'Sub Category'" v-on:searchedData="refresh_data" :practice_id="practice_id" :field_wdth="250" :rounded_px="10"></search-database-form-control>
                                        </div>
                                        <div class="width-20-pc pull-right mg-bottom-10">
                                            <div class="btn-group float-right" role="group" aria-label="Button group with nested dropdown">
                                                <button data-toggle="modal" data-target="#new_category_modal" type="button" class="btn combo-button combo-default">+ Sub-category</button>
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="btnGroupDrop">
                                                        <a class="dropdown-item" data-toggle="modal" data-target="#import_sub_categories_modal_id"> Import Sub-category</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <table class="receipt-item-table-2 table table-bordered table-hover mg-top-5">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width: 15%;">
                                                        <a @click="loadSubCategories" title="Refresh" class="fa fa-refresh float-right pointer-cursor"></a>
                                                    </th>
                                                </tr>
                                            </thead>
                                            <tbody v-if="subcategories.length">
                                                <tr v-for="(category, index) in subcategories" :key="'category_'+index" class="pointer-cursor">
                                                    ...
                                                    <delete-modal :modal_title="'Delete sub category'" :modal_id="'delete_category_'+index" v-on:deletionSuccessful="remove_el(category)" :confirm_message="'Are you sure?'" :item_url="initial_url+'/'+category.id"></delete-modal>
                                                    <sub-category-modal v-if="page_ready" :form_data="category" :INITIAL_URL="initial_url+'/'+category.id" :item_id="category.id" :user_mode="'Edit'" :title="'+ New Category'" :modal_id="'new_category_modal_'+index" v-on:categoryEdited="replace_edited" :practice_id="practice_id"></sub-category-modal>
                                                </tr>
                                            </tbody>
                                            <tbody v-if="!subcategories.length">
                                                <tr>
                                                    <td colspan="5" class="cl-amref text-center fs-16 padding-top-20 padding-bottom-20">No sub category to display</td>
                                                </tr>
                                            </tbody>
                                            <tfoot>
                                                <tr>
                                ...
                                <sub-category-modal v-if="page_ready" :user_mode="'Create'" :INITIAL_URL="initial_url" :title="'+ New Sub Category'" :modal_id="'new_category_modal'" v-on:categoryAdded="loadSubCategories" :practice_id="practice_id"></sub-category-modal>
                                <general-import-modal v-if="page_ready" v-on:imported="loadSubCategories" :practice_id="practice_id" :title="'Import Sub Category'" :modal_id="'import_sub_categories_modal_id'" :modal_min_width="900" :sample_file="'subcategory'" :upload_type="'Sub Category'"></general-import-modal>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    import SubCategoryModal from "../../partials/modals/inventory_settings/category/SubCategoryModal";
    ...
    export default {
        ...
        DeleteModal,SubCategoryModal,GeneralImportModal,SearchDatabaseFormControl,
