                ...
                <div class="col-md-12 col-lg-12 col-sm-12 padding-left-0">
                    ...
                    <div class="row">
                        <div class="col-md-2 col-lg-2 col-sm-2 padding-right-0 bg-foo content-calculated-height-wc">
                            <side-bar-inventory v-if="page_ready" :supplier="true" :scrollable="true" :width="true" :transparent="true" :active_num="'bills'" :title="'Vendors'"></side-bar-inventory>
                        </div>
                                        ...
                                        <div v-else class="btn-group" role="group" aria-label="Button group">
                                            <div class="btn-group" role="group">
                                                ...
                                                <div class="dropdown-menu" :aria-labelledby="'btnGroupDrop2_1'">
                                                    ...
                                                    <a class="dropdown-item pointer-cursor">Unpaid</a>
                                                    <a class="dropdown-item pointer-cursor">Partial Paid</a>
                                            ...
                                            <div class="padding-right-10 min-height-30 width-100-pc">
                                                ...
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <div class="dropdown-menu" :aria-labelledby="'btnGroupDrop2'">
                                                        ...
                                                        <a class="dropdown-item pointer-cursor">Bill#</a>
                                                        <a class="dropdown-item pointer-cursor">Vendor Name</a>
                                                        ...
                                                        <a class="dropdown-item separator"></a>
                                                        <a class="dropdown-item pointer-cursor">Import Bills</a>
                                                        <a class="dropdown-item pointer-cursor">Export Bills</a>
                                                        <a class="dropdown-item separator"></a>
                                    ...
                                    <div class="dgrid dgrid-grid ui-widget dgrid-03 mg-bottom-30 universal-grid border-ccc" role="grid">
                                                ...
                                                <table class="table table-vendor-list table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th style="width:13%;" class="text-right">Status</th>
                                                    ...
                                                    <tbody>
                                                        <tr v-for="(bill,index) in bills" :key="'purchase_order_'+index">
                                                            <td>
                                                                <input class="pointer-cursor" type="checkbox" v-model="selected_pos" :value="bill.id">
                                                            </td>
                                                            <router-link tag="td" :to="'/purchases/purchase_orders/'+bill.id+'/view'" class="padded pointer-cursor txt-uppercase">
                                                                {{bill.bill_date}}
                                                            </router-link>
                                                            <router-link tag="td" :to="'/purchases/bills/'+bill.id+'/view'" class="padded pointer-cursor cl-amref fw-600 txt-uppercase">
                                                                {{bill.trans_number}}
                                                            </router-link>
                                                            <router-link tag="td" :to="'/purchases/bills/'+bill.id+'/view'" class="padded pointer-cursor">
                                                                {{bill.ref_number}}
                                                            </router-link>
                                                            <!-- <router-link tag="td" :to="'/purchases/bills/'+bill.id+'/view'" class="padded pointer-cursor">
                                                                {{bill.vendor.display_as}}
                                                            </router-link>
                                                            <router-link tag="td" :to="'/purchases/bills/'+bill.id+'/view'" class="padded pointer-cursor fw-600 fs-12 cl-444 txt-uppercase text-right">
                                                                {{bill.vendor.currency.currency_sympol+' '+format_Money(bill.total-bill.discount_allowed)}}
                                                            </router-link>
                                                            <router-link tag="td" :to="'/purchases/bills/'+bill.id+'/view'" class="padded pointer-cursor txt-uppercase">
                                                            ...
                                                            <router-link tag="td" :to="'/purchases/bills/'+bill.id+'/view'" class="padded pointer-cursor">
                                                                <a v-if="check_status(bill.status,PROCESS_STATUS.ACCEPTED)" class="txt-uppercase fs-12 fw-600 cl-success-light">Accepted</a>
                                                                <a v-else-if="check_status(bill.status,PROCESS_STATUS.DRAFT)" class="txt-uppercase fs-12 fw-600 cl-787887">Draft</a>
                                                                <a v-else-if="check_status(bill.status,PROCESS_STATUS.DECLINED)" class="txt-uppercase fs-12 fw-600 cl-amref">Declined</a>
                                                                <a v-else-if="check_status(bill.status,PROCESS_STATUS.EXPIRED)" class="txt-uppercase fs-12 fw-600 cl-787887">Expired</a>
                                                                <a v-else-if="check_status(bill.status,PROCESS_STATUS.INVOICED)" class="txt-uppercase fs-12 fw-600 cl-success-light">Invoiced</a>
                                                                <a v-else-if="check_status(bill.status,PROCESS_STATUS.SENT)" class="txt-uppercase fs-12 fw-600 cl-blue-link">Sent</a>
                                                                <a v-else-if="check_status(bill.status,PROCESS_STATUS.OPEN)" class="txt-uppercase fs-12 fw-600 cl-blue-link">Open</a>
                                        ...
                                        <div v-else class="row">
                                            ...
                                            <div class="col-md-12 col-lg-12 bg-white text-center border-bottom mg-top-30 padding-top-30">
                                                <p class="fs-20">Owe money? It's good to pay bills on time!</p><br>
                                                <small class="fs-14 cl-777">If you've purchased something for your business, and you don't have to repay it immediately, then you can record it as a bill.</small><br><br>
                                                <a data-toggle="modal" data-target="#new_po_modal" class="btn btn-default btn-inventory btn-amref cl-white txt-uppercase">Create a Bill</a>
                                            </div>
...
<script>
    ...
    import {COUNTRIES_URL,SUPPLIER_URL,SUPPLIER_BILL_URL,ACCOUNTS_INITIALS,SUPPLIER_COMPANIES_URL,SUPPLIER_PO_URL} from '../../../../../router/api_routes';
    export default {
        name: "BillIndex",
