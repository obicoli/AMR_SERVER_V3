<template>
    <div class="width-400-px">
        <label v-if="show_label" class="fs-12"><i class="fa fa-barcode" aria-hidden="true"></i> SCAN BARCODE OR SEARCH ITEMS</label>
        <input type="text" :placeholder="placeholder" class="report-filters-inputs height-32" v-bind:style="{width:field_wdth+'px',borderRadius:rounded_px+'px'}" @keyup="search_things($event,'practice_product_item')" autofocus="autofocus" />
...
<script>
    ...
    export default {
        props: ['practice_id','field_wdth','placeholder','rounded_px','show_label','search_result_width','search_result_min_height'],
        data(){
            return {
