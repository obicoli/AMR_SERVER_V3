        ...
        <div class="width-100-pc float-left invoice invoice-2 invoice-body mg-left-5 mg-right-10 mg-top-5 invoice-border zero-border padding-20 max-width-800" :id="'receipt_'+payment.id">
            <div class="width-100-pc float-left">
                ...
                <div class="width-30-pc float-right">
                    <h2 class="invoice-title txt-uppercase fs-20 width-100-pc text-right"><b>{{payment.document_name}}</b></h2>
                    <table class="table overview-table">
                        <tbody>
                            <tr>
                                <td style="width:50%" class="text-right">Payment Date:</td>
                                <td class="text-right fw-600">{{payment.trans_date}}</td>
                            </tr>
                            <tr>
                ...
                <div class="width-30-pc float-left mg-top-20">
                    <h3 class="fw-600 cl-444 width-100-pc fs-14 mg-left-10 txt-uppercase">Customer:</h3>
                    <table class="sales-table width-100-pc">
                        <tr class="info">
                            <td colspan="2" class="width-100-pc no-bd">
                                {{payment.customer.legal_name}}<br>
                                {{payment.customer.addresses.billing.address}}, {{payment.customer.addresses.billing.postal_code}}<br>
                                {{payment.customer.addresses.billing.phone}} | {{payment.customer.email}}<br>
                                {{payment.customer.addresses.billing.fax}}<br>
                            </td>
                        </tr>
                    </table>
                </div>
                <div class="width-100-pc float-left">
                    <div v-if="balance" class="width-100-pc float-left mg-top-20 padding-20">
                        <h3 class="width-100-pc cl-444 fs-14 mg-bottom-15 fw-600 txt-uppercase">Details:</h3>
                        <table class="payment-receipt-table width-100-pc">
                            <tr class="title">
                                <td class="" style="width:40%;">Item/Goods/Service/Description#</td>
                                <td class="text-right" style="width:15%;">Payment Date</td>
                                <td class="text-right" style="width:15%;">Total Amount</td>
                                <td class="text-right" style="width:15%;">Payment Amount</td>
                                <td class="text-right" style="width:15%;">Balance</td>
                            </tr>
                            <tr v-for="(pay_bill,index) in payment.items" class="info" :key="'pay_bill_'+index">
...
<script>
    ...
    export default {
        name: "PaymentReceivedPreview",
        props: ['payment_received','currency','show_actions','balance'],
        components: {
            CompanyLogo
        },
        data(){
        ...
        watch: {
            payment_received: function(new_data,old_data){
        ...
        mounted(){
            if(this.payment_received){
                this.payment = this.payment_received;
                console.info(this.payment.items);
